...
@section('title', __('messages.skill_assessment_exams') ?? 'Skill Assessment Exams')
                ...
                <div class="card">
                    <div class="card-body">
                        <h5 class="card-title">{{ __('messages.manage_exams') ?? 'Manage Exams' }}
                            <a href="{{ route('business.skill-assessment.exams.create') }}" class="btn btn-primary"
                                style="float: right;">
                                            ...
                                            <tbody>
                                                @foreach ($examTemplates as $index => $template)
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <a href="{{ route('business.skill-assessment.sections') }}?exam_template_id={{ $template->id }}"
                                                        ...
                                                        <td>
                                                            <a href="{{ route('business.skill-assessment.exams.edit', ['id' => $template->id]) }}"
                                                            ...
                                                            <a href="javascript:void(0);"
                                                                ...
                                                                onclick="return showConfirmToast('{{ route('business.skill-assessment.exams.delete', ['id' => $template->id]) }}')"
                                                                class="text-danger">
...
@section('customjs')
    <script type="text/javascript">
        function update_status(el) {
            ...
            $.post('{{ route('business.skill-assessment.exams.status') }}', {
                _token: '{{ csrf_token() }}',
                id: el.value,
                status: status
            }, function(data) {
